...
export async function piSearch(q:string,k:string,s:string) {
  ...
  try{
    ...
    const tracks=eps.slice(0,15).map(e=>{const dur=(e.duration as number)??0;const streamURL=(e.enclosureUrl as string)??null;if(streamURL)cacheSet(`pi:str:${e.id}`,streamURL,86400);return{id:`pi_ep_${e.id}`,title:(e.title as string)??"Unknown",artist:(e.feedTitle as string)??"Unknown",album:(e.feedTitle as string)??null,duration:dur||null,artworkURL:(e.image??e.feedImage) as string|null,format:"mp3",streamURL};});
...
export async function taddySearch(q:string,k:string,uid:string) {
  ...
  try{
    ...
    const tracks=((sr.podcastEpisodes as Record<string,unknown>[])??[]).map(e=>{const dur=(e.duration as number)??0;const ser=e.podcastSeries as Record<string,unknown>|null;const streamURL=(e.audioUrl as string)??null;if(streamURL)cacheSet(`taddy:str:taddy_ep_${e.uuid}`,streamURL,86400);return{id:`taddy_ep_${e.uuid}`,title:(e.name as string)??"Unknown",artist:(ser?.name as string)??"Unknown",album:(ser?.name as string)??null,duration:dur||null,artworkURL:(e.imageUrl as string)??null,format:"mp3",streamURL};});
    const r={tracks,albums}; cacheSet(ck,r,300); return r;
  }catch{return{tracks:[],albums:[]};}
}
